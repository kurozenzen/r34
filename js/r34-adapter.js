// config
let serviceUrl = "https://rule34-api.netlify.app";
const autoCompleteUrl = "https://api.rule34.xxx/autocomplete.php";
...
app.controller("r34Ctrl", function ($http) {
  ...
  controller.init = function () {
    ...
    //         let replacementService = 'https://rule-34-api-vercel.vercel.app';
  ...
  controller.getPosts = function (mode) {
    ...
    if ($("#likes").prop("checked")) tagArray.push("sort:score");
    let tags = "";
    let page = "";
    ...
    //rule34-api.netlify.app/posts?limit=20&pid=0&tags=sort:id:desc+yuri+lesbian+clothing+

    https: $http
      .get(serviceUrl + "/posts?limit=" + controller.pageSize + tags + page)
      .then(function (response) {
        ...
        for (let post of controller.posts) {
  ...
  controller.tagClass = function (tag) {
    ...
    if (tag.startsWith("source:")) return "source";
    if (tag.startsWith("rating:")) return "rating";
    return "";
  };

  controller.tagName = function (tag) {
    if (tag.includes(":")) return tag.split(":")[1];
  ...
  controller.getSuggestions = function () {
    ...
    if (search.length > controller.awesomplete.minChars) {
      $http
        .get(autoCompleteUrl + "?q=" + search)
        .then(function (response) {
          controller.awesomplete.list = response.data;
        })
        .catch(function (error) {
          console.log(error);
        });
  ...
  controller.getAlias = function () {
    ...
    for (let activeTag of controller.activeTags) {
      $http
        .get(serviceUrl + "/alias/" + activeTag)
        .then(function (response) {
          controller.alias.push(
            ...response.data.filter(
              (t) => !controller.activeTags.includes(t.name),
            ),
          );
        })
        .catch(function (error) {
  ...
  controller.vote = function (postId, type) {
    $http
      .get(
        "https://rule34.xxx/index.php?page=post&s=vote&id=" +
          postId +
          "&type=" +
          type,
      )
      .then(function (response) {
        controller.posts.find((post) => post.id === postId).score = parseInt(
          response.data,
        );
      })
      .catch(function (error) {
  ...
  controller.infiniteScrollDisabled = function () {
    return (
      $("#infiniteScroll").prop("checked") !== true || controller.noImagesLeft
    );
  };
});
...
function tagify(text) {
  ...
  return text.toLowerCase().replace(/ /g, "_").replace("-", "");
}
